...
use drv_spi_api::*;
use idol_runtime::{Leased, LenLimit, RequestError, R, W, LeaseBufReader, LeaseBufWriter};
use ringbuf::*;
...
impl ServerImpl {
    fn ready_writey(
        ...
        let mut tx: Option<LeaseBufReader<_, BUFSIZ>> = data_src.map(|b| LeaseBufReader::from(b.into_inner()));
        let mut rx: Option<LeaseBufWriter<_, BUFSIZ>> = data_dest.map(|b| LeaseBufWriter::from(b.into_inner()));
