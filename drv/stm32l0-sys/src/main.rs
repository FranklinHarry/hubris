...
use userlib::*;
use drv_stm32l0_sys_api::{RccError, GpioError, Port};
...
macro_rules! do_set_reset {
    ($reg:expr, $set:expr, $reset:expr) => {
        $reg.bsrr.write(|w| unsafe { w.bits((u32::from($reset) << 16) | u32::from($set)) })
    };
}
...
mod idl {
    use super::{RccError, GpioError, Port};
